...
source_sqlite_offline = new ol.source.XYZ({
    tileUrlFunction: function (tileCoord) {
        if (tileCoord) {
            ...
            return "http://" + window.location.host + "/MBTilesServlet?T=GMap&L=" + z + "&X=" + x + "&Y=" + y;
        } else {
            return '';
        }
    }
...
source_sqlitesat_offline = new ol.source.XYZ({
    tileUrlFunction: function (tileCoord) {
        if (tileCoord) {
            ...
            var y = -tileCoord[2] - 1;
            y = 2 * (Math.pow(2, (z - 1)) - y) - 1 + y;
            //"http://" + window.location.host + window.location.pathname + "MBTilesServlet?
            return "http://" + window.location.host + "/MBTilesServlet?T=GMapSat&L=" + z + "&X=" + x + "&Y=" + y;
        } else {
            return '';
        }
    }
